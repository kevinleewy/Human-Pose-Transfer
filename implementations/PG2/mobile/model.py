...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, activation=nn.ReLU):
        ...
        depthwise = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, stride = stride, padding=padding, groups=in_channels)
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, padding=1):
        super().__init__()
        self.main = nn.Sequential(
            DepthwiseSeparableConv(in_channels, in_channels, kernel_size, stride, padding, activation=nn.ReLU),
            DepthwiseSeparableConv(in_channels, in_channels, kernel_size, stride, padding, activation=nn.ReLU),
        )
...
class _G1EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, is_last_block=True):
        ...
        if not self.is_last_block:
            self.conv = DepthwiseSeparableConv(in_channels, out_channels, 3, 2, 1, activation=nn.ReLU)
...
class _G1DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, is_last_block=True):
        ...
        self.is_last_block = is_last_block
        if not self.is_last_block:
            self.conv = DepthwiseSeparableConv(in_channels, out_channels, 1, 1, 0, activation=nn.ReLU)
...
class Generator1(nn.Module):
    def __init__(self, in_channels, num_repeat, middle_features_dim, channels_base, image_size=(128, 64)):
        ...
        self.start_conv = DepthwiseSeparableConv(in_channels, channels_base, kernel_size=3, stride=1, padding=1, activation=nn.ReLU)
        ...
        self.end_conv = DepthwiseSeparableConv(in_channels_list[-1], 3, kernel_size=3, stride=1, padding=1, activation=None)
...
class _G2EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, is_last_block=True):
        ...
        super().__init__()
        self.main = nn.Sequential(
            DepthwiseSeparableConv(in_channels, out_channels, 3, 1, 1, activation=nn.ReLU),
            DepthwiseSeparableConv(out_channels, out_channels, 3, 1, 1, activation=nn.ReLU)
        )
        self.is_last_block = is_last_block
        if not self.is_last_block:
            self.conv = DepthwiseSeparableConv(out_channels, out_channels, 3, 2, 1)
...
class Generator2(nn.Module):
    def __init__(self, in_channels, channels_base, num_repeat, num_skip_out_connect=0, weight_init_way=None):
        ...
        self.end_conv = DepthwiseSeparableConv(channels_base, 3, kernel_size=3, stride=1, padding=1, activation=None)
